...
gcHeader* gcRawAlloc(int size, int refCount){
    ...
    return new_obj;//Возвращается сырой указатель Если потерять его, или сборка мусора начнется до того, как этот указатель будет скопирован куда надо, всё станет плохо
}
